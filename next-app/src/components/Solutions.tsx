...
const Solutions = () => {
  const solutions = [
    { title: 'Automated Fare Collection', desc: 'Flexible payment systems that support contactless, card, and mobile transactions across transit networks.', icon: (
    ...
    { title: 'Intelligent Transport Systems', desc: 'Real-time monitoring and control across vehicles, routes, and passenger information.', icon: (
    ...
    { title: 'AI-Based Bus Scheduling', desc: 'Plan and adjust services using real-time and historical data for optimized fleet utilization.', icon: (
    ...
    { title: 'Control Room Management', desc: 'Manage operations from a single command environment with live tracking and insights.', icon: (
    ...
    { title: 'Micro-Mobility Integration', desc: 'Extend transit with connected first and last mile services integrated into the core backbone.', icon: (
    ...
    { title: 'Performance Monitoring', desc: 'Comprehensive analytics for operators to track network health and efficiency in real-time.', icon: (
  ...
  return (
    <section id="solutions" style={{ background: '#0a1e3d', padding: '120px 48px', borderTopLeftRadius: 48, borderTopRightRadius: 48 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          ...
          {solutions.map((s, i) => (
            <div key={i} className="solution-card" style={{
              ...
              <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 19, fontWeight: 600, marginBottom: 12, lineHeight: 1.3 }}>{s.title}</h3>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 14.5, lineHeight: 1.7, margin: 0 }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
